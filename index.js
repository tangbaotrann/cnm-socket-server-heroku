...
io.on("connection", (socket) => {
    ...
    socket.on("send_message", ({ message }) => {
        //console.log(message);
        try {
            const { conversationID, contentMessage, content, createAt } =
                message;
            const conversation = {
                conversationID,
                content,
    ...
    socket.on("block_message_user_in_group", ({ info }) => {
        ...
        try {
            ...
            if (blockByUserId) {
                io.to(blockByUserId.socketId).emit(
                    "blocked_message_user",
                    arrBlocked
                );
            }
        } catch (error) {
